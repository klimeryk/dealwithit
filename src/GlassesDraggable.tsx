...
function GlassesDraggable({ glasses, inputImageRef }: GlassesDraggableProps) {
  ...
  function handleDragMove({ delta }: DragMoveEvent) {
    ...
    const maxWidthRelativeToParent = inputImageRef.current.width - glasses.coordinates.x;
    ...
    const maxHeightRelativeToParent = inputImageRef.current.height - glasses.coordinates.y;
  ...
  } else if (inputImageRef.current && positionX + glasses.size.width > inputImageRef.current.width) {
    clipPath.right = Math.abs(positionX + glasses.size.width - inputImageRef.current.width);
  }

  const positionY = glasses.coordinates.y + (transform?.y || 0);
  if (positionY < 0) {
    clipPath.top = Math.abs(positionY);
  } else if (inputImageRef.current && positionY + glasses.size.height > inputImageRef.current.height) {
    clipPath.bottom = Math.abs(positionY + glasses.size.height - inputImageRef.current.height);
  }
  ...
  return (
    <span className="absolute w-1/2 left-0 top-0 touch-none" ref={setDraggableRef} style={glassesStyle}>
      <img
        alt="Draggable glasses"
